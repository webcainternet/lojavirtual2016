<div class="form-group <?php if ($required) { ?>required<?php } ?>">
    ...
    <div class="<?php echo $page == 'checkout' ? 'col-sm-9' : 'col-sm-10' ?>">
      ...
      <?php } else { ?>
        <input  class="form-control" type="<?php echo $type == 'date' ? 'text' : $type ?>" name="<?php echo $name ?>" id="<?php echo $id ?>" value="<?php echo $value ?>" placeholder="<?php echo $placeholder ?>" <?php echo $attrs ?> <?php echo $reload ? 'data-onchange="reloadAll"' : 'data-reload-payment-form="true"'?>>
      <?php } ?>
      <?php if (!empty($rules)) { ?>
